...
People need to be added to the book. When our book was unsorted we could add them to the end of the book without thinking about where to put them. Since it's sorted we need to determine the right place to put them. This is the first request that is harder than it used to be. It's not a lot harder, but it is harder.
...
Remember people need their info updated, they need it added, and they need it removed. We have to keep all copies of the phone book current. We also need to maintain the sorting in each. Any change data request becomes slower and more difficult with each copy.

The other queries get slower too. Basic lookups must wait until we finish updates to ensure accuracy. The request queue is piling up.
