...
For any algorithm, there will be an operation happening alongside the data. Big-O implies operation_time so can actually omit it. Now we have a very simple expression.
...
Logarithmic time is faster than n time. That may seem weird at first to think that something can be faster than n. But if we visualize an example it's clearly possible and it's very important.
...
Without a logarithmic way to whittle down the data set, it would be impossible. If the "asking questions algorithm" was _O(n)_ you would have a 20 in n chance of winning, where n is all the data in the person's brain. But since asking questions is like _O(log n)_ it's pretty easy to win.
...
It's better to ask questions that will roughly cut the data set in half. When writing code you can always cut the list in half until you get to a single item.
...
When thinking of _O(log n)_ think of binary search. Think of cutting a dataset in half like the 20 questions game.
...
For our purposes this is a divide and conquer algorithm. Divide (split) is _O(log n)_. Conquer (merge) is _O(n)_. Divide and conquer is a useful simplification for this level of overview. Not all divide and conquer algorithms are _O(n log n)_ but when the conquer step is _O(n)_ you can be pretty sure the whole algorithm is _O(n log n)._
